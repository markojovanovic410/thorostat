...
export class RacesListComponent implements OnInit {
  ...
  loadRaces(evt) {
    ...
    this._raceService.getByParams<any>({race_date: this.race_date, track_code: this.track_code, race_num_only: true}).subscribe(
      res => {
        if (res.success)
  ...
  loadRaceEntries(evt) {
    ...
    this._raceService.getByParams<any>({race_date: this.race_date, track_code: this.track_code, race_num: this.race_num}).subscribe(
      res => {
        if (res.success) {
          this.race_details = res.races[0];
          this.recommendation = this.race_details.entries.reduce((prev, current) => ((parseInt(prev.lifetime_wins)/parseInt(prev.lifetime_sts)) > (parseInt(current.lifetime_wins)/parseInt(current.lifetime_sts))) ? prev : current);
        }
  ...
  formatWorkout(workout) {
    ...
    return date + " " + workout.track_code + " " + distance + " " + workout.condition + " " + time + " " + workout.description + " (" + workout.rank_among_others + " / " + workout.number_of_workout + ")";
  }
